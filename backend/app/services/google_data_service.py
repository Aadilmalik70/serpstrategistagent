...
def _oauth_config() -> tuple[str, str, str, str]:
    client_id = os.getenv("GOOGLE_INTEGRATION_CLIENT_ID", "").strip()
    client_secret = os.getenv("GOOGLE_INTEGRATION_CLIENT_SECRET", "").strip()
    redirect_uri = os.getenv("GOOGLE_INTEGRATION_REDIRECT_URI", "").strip()
...
async def get_connection(db: AsyncSession, workspace_id: uuid.UUID, user_id: uuid.UUID) -> GoogleDataConnection | None:
...
async def start_google_oauth(db: AsyncSession, workspace_id: uuid.UUID, user_id: uuid.UUID) -> str:
...
async def list_google_properties(db: AsyncSession, connection: GoogleDataConnection) -> tuple[list[GooglePropertyOption], list[GooglePropertyOption]]:
    token = await _access_token(db, connection)
    headers = {"Authorization": f"Bearer {token}"}
    async with httpx.AsyncClient(timeout=20.0) as client:
        gsc_response = await client.get("https://www.googleapis.com/webmasters/v3/sites", headers=headers)
